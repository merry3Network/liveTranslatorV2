...
## 🛠️ 便利な管理・確認コマンド (PM2 / System)

稼働状況やトラブルシューティング時に使用する主要コマンドです。

### PM2 (プロセス管理)
| コマンド | 内容 |
|---|---|
| `pm2 status` | 全プロセスの稼働状況、CPU/メモリ使用率の一覧表示 |
| `pm2 logs` | 全プロセスのリアルタイムログ（エラーの特定に必須） |
| `pm2 show [name]` | 特定のプロセス（例: `live-translator`）の詳細情報を表示 |
| `pm2 monit` | ターミナル上のグラフィカルなモニター画面を起動 |
| `pm2 restart all` | 全プロセスの再起動 |

### システム確認
| コマンド | 内容 |
|---|---|
| `hostname -I` | VM の IP アドレスを素早く確認 |
| `top` / `htop` | OS 全体のリソースモニター (CPU/RAM) |
| `df -h` | ディスク残容量の確認 |
